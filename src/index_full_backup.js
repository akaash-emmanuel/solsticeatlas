...
import { createCometSystem, updateComets } from './components/CometSystem.js';
import { showAstronautToolsMenu } from './components/AstronautTools.js';
...
let renderer, camera, scene, controls, cameraManager;
let Globe;
let globeGroup;
...
function init() {
  ...
  initNeptune();
  initAsteroidBelt();
  window.cometSystem = createCometSystem(scene, sun.position);
...
function animate() {
  ...
  // Update comets
  if (sun && window.cometSystem) {
    updateComets(sun.position, deltaTime);
  }
...
function createButtons() {
  ...
  astronautButton.addEventListener("click", () => {
    ...
    AstronautTools.showAstronautToolsMenu(scene, Globe, globeGroup, camera, {
      mars, jupiter, saturn, venus, uranus, neptune, mercury, moon
    });
...
function switchCameraFocus(target, duration = 1.5) {
  ...
  // Store the target for use in UI updates
  ...
  switch (cameraFocus) {
    ...
    case 'moon':
      targetObject = moon;
      viewDistance = moon ? moon.geometry.parameters.radius * 3 : 20;
      break;
    case 'sun':
      targetObject = sun;
      viewDistance = 1000;
    ...
    case 'mercury':
      ...
      viewDistance = mercury ? mercury.geometry.parameters.radius * 3.5 : 50;
      break;
    case 'venus':
      targetObject = venus;
      viewDistance = venus ? venus.geometry.parameters.radius * 3.5 : 80;
      break;
    case 'mars':
      targetObject = mars;
      viewDistance = mars ? mars.geometry.parameters.radius * 3.5 : 40;
      break;
    case 'jupiter':
      targetObject = jupiter;
      viewDistance = jupiter ? jupiter.geometry.parameters.radius * 3.5 : 200;
      break;
    case 'saturn':
      targetObject = saturn;
      viewDistance = saturn ? saturn.geometry.parameters.radius * 3.5 : 180;
      break;
    case 'uranus':
      targetObject = uranus;
      viewDistance = uranus ? uranus.geometry.parameters.radius * 3.5 : 100;
      break;
    case 'neptune':
      targetObject = neptune;
      viewDistance = neptune ? neptune.geometry.parameters.radius * 3.5 : 100;
      break;
    case 'asteroidbelt':
      // For asteroid belt, we can't focus on a single object in the same way,
      // or we focus on Sun but zoom out? 
      // Existing logic used Sun position but specific offset.
      // Let's focus on Sun but with large distance? 
      // Or create a dummy target object?
      // Use Sun for now as anchor.
      targetObject = sun;
      viewDistance = 3000;
      break;
    default:
  ...
  // If we have a target object, get its position (unless already set for asteroid belt)
  if (targetObject && cameraFocus !== 'asteroidbelt') {
    ...
    // If we're too close or inside, back out along the current vector
  ...
  function animateCamera() {
    ...
    if (progress < 1) {
      ...
      // Apply "Moon-like" controls to all planets for better UX
      if (cameraFocus === 'sun' || cameraFocus === 'asteroidbelt') {
        // Standard controls for Sun and wide views
        controls.minDistance = 400;
        controls.maxDistance = 50000;
        controls.enableDamping = false;
      } else {
        // Enhanced controls for all planets (Earth, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune)
        // Calculate approximate radius for constraint setting
        let radius = 10; // Default fallback

        if (cameraFocus === 'earth') radius = 100; // Approx Earth scale
        else if (cameraFocus === 'moon' && moon) radius = moon.geometry.parameters.radius;
        else if (cameraFocus === 'mercury' && mercury) radius = mercury.geometry.parameters.radius;
        else if (cameraFocus === 'venus' && venus) radius = venus.geometry.parameters.radius;
        else if (cameraFocus === 'mars' && mars) radius = mars.geometry.parameters.radius;
        else if (cameraFocus === 'jupiter' && jupiter) radius = jupiter.geometry.parameters.radius;
        else if (cameraFocus === 'saturn') radius = SATURN_RADIUS * 30;
        else if (cameraFocus === 'uranus') radius = URANUS_RADIUS * 30;
        else if (cameraFocus === 'neptune') radius = NEPTUNE_RADIUS * 30;

        // Apply constraints allowing close inspection but preventing clipping
        controls.minDistance = radius * 1.5;
        controls.maxDistance = radius * 20.0; // Allow reasonable zoom out

        controls.enableDamping = false; // direct control
        controls.enableZoom = true;
        controls.autoRotate = false;
        controls.rotateSpeed = 0.8;
      }
